...
const extension: JupyterFrontEndPlugin<void> = {
  ...
  activate: async (app: JupyterFrontEnd, settings: ISettingRegistry) => {
    ...
    function loadSetting(setting: ISettingRegistry.ISettings): void {
      ...
      if (
        (!nbgallery_url || nbgallery_url.length == 0) &&
        env_nbgallery_url &&
        env_nbgallery_url.length > 0
      ) {
        nbgallery_url = env_nbgallery_url;
        setting.set("nbgallery_url", nbgallery_url);
      }

      nbgallery_client_name = setting.get("nbgallery_client_name")
        .composite as string;
      if (
        (!nbgallery_client_name || nbgallery_client_name.length == 0) &&
        env_nbgallery_client_name &&
        env_nbgallery_client_name.length > 0
      ) {
