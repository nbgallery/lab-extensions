...
function inject(panel: NotebookPanel, gallery_metadata: any): void {
  ...
  if (kernel.name == "python" || kernel.name == "python3") {
    kernel.requestExecute({
      code:
        "import os; os.environ['NBGALLERY_UUID']='" +
        gallery_metadata["uuid"] +
        "'; os.environ['NBGALLERY_GIT_COMMIT_ID']='" +
        gallery_metadata["git_commit_id"] +
        "';",
      silent: true,
      stop_on_error: true,
    });
  ...
  if (kernel.name == "ruby") {
    kernel.requestExecute({
      code: "ENV['NBGALLERY_UUID']='" + gallery_metadata["uuid"] + "'",
      silent: true,
      stop_on_error: true,
    });
    kernel.requestExecute({
      code:
        "ENV['NBGALLERY_GIT_COMMIT_ID']='" +
        gallery_metadata["git_commit_id"] +
        "'",
      silent: true,
      stop_on_error: true,
  ...
  panel.sessionContext.ready.then(() => {
    ...
    if (gallery_metadata && gallery_metadata["uuid"]) {
      ...
      panel.sessionContext.statusChanged.connect(() => {
        ...
        if (
          panel.sessionContext.session.kernel.status == "unknown" &&
          restarting
        ) {
